...
declare global {
  interface Window {
    electronAPI: {
      ...
      getFileStats: (filePath: string) => Promise<{ ok: boolean; size?: number; mtimeMs?: number; error?: string }>;
...
class ElectronService {
   ...
   */
  async getFileStats(filePath: string): Promise<{ size: number; mtimeMs: number }> {
    try {
      const api = this.getAPI();
